...
ctest.sh [-a; -s] <progpath> <inputs_folder_path> <expected_outputs_folder_path> 
...
for file in $(ls $INPUTS); do
	...
	if [[ $ARGS == 'true' ]] && [[ $STDIN == 'true' ]]; then
		...
		output=$(tail -n $((lines-1)) $inputfile | $PROGRAM $(head -n 1 $inputfile))
	# otherwise, use the whole file as either the stdin or arguments
	elif [[ $STDIN == 'true' ]]; then
		output=$(cat $inputfile | $PROGRAM)
	elif [[ $ARGS == 'true' ]]; then
		output=$($PROGRAM $(cat $inputfile))
	else
